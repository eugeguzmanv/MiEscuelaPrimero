...
const ApoyoModel = {
    ...
    updateApoyoEstatus: async (idApoyo, nuevoEstatus) => {
        return db('Apoyo').where({ idApoyo }).update({ estatus: nuevoEstatus });
    },
    ...
    getApoyosPendientes: async () => {
        return db('Apoyo')
            .where({ estatus: 'Pendiente' })
            .select('idApoyo', 'tipo', 'categoria', 'descripcion', 'estatus');
    },
